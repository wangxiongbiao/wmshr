...
export function EmployeeModal({ isOpen, onClose, onSave, employee }: EmployeeModalProps) {
  const [formData, setFormData] = useState<Partial<Employee>>({
    name: '', gender: 'female', country: 'MM', role: '拣货员', dept: '', hourlyRate: undefined, baseMonthlyWage: undefined, currency: 'THB', joinDate: new Date().toISOString().split('T')[0], status: '在职', photo: null
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | React.SelectElement>) => {
    ...
    if (name === 'hourlyRate' || name === 'baseMonthlyWage') {
