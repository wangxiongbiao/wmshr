...
export function SopManager({ employees, addToast }: SopManagerProps) {
  ...
  const handleMarkAsReadInSimulator = (sopId: string) => {
    ...
    const updatedSop = updatedSops.find(s => s.id === sopId);
    if (updatedSop) {
      setCurrentSelectedSimSop(updatedSop);
      if (selectedSopForDetails?.id === sopId) {
        setSelectedSopForDetails(updatedSop);
      }
    }
  };
                ...
                <div className="pt-2">
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-lg text-xs transition shadow-md hover:shadow-lg flex items-center justify-center gap-1.5"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      <span>{editingSopId ? "保存并完成对口同步发布" : "立即全端一键通知下发"}</span>
                    </button>
                  </div>
                </div>
