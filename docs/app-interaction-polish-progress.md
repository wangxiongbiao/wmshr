...
- 考勤记录：列表页直接承载查看职责，移除详情页导航与废弃文件，补齐记录状态标识与备注占位；按最新要求移除显式 loading 卡片，并改为基于 `limit + offset` 的分页加载更多。
- SOP 列表：补齐已读状态回流提示；按最新要求移除显式 loading 卡片，并改为基于 `limit + offset` 的分页加载更多。
