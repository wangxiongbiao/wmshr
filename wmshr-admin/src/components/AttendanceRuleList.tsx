  ...
  const filteredRules = useMemo(() => {
    ...
    return rules.filter((rule) => {
      ...
      if (effectiveStatus !== "all") {
        const effectiveKey =
          effective.label === "未开始" ? "upcoming" : effective.label === "已过期" ? "expired" : "active";
  ...
  return (
    <div className="space-y-6">
      <div className="glass-panel rounded-xl p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="grid flex-1 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(320px,1.4fr)_180px_180px]">
            <div>
              <label className="mb-1 block text-xs font-medium uppercase text-slate-500">搜索规则</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium uppercase text-slate-500">启用状态</label>
              <select
                value={isActive}
                onChange={(event) => {
            ...
            <div>
              <label className="mb-1 block text-xs font-medium uppercase text-slate-500">有效期状态</label>
              <select
          ...
          <div className="flex flex-col gap-2 xl:w-[220px] xl:flex-shrink-0">
            ...
            <div className="flex gap-3 xl:grid xl:grid-cols-2">
              ...
              >
                <Plus className="hidden h-4 w-4 xl:block" /> 新增
              </button>
            </div>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel rounded-xl p-10 text-center text-sm text-slate-500">当前筛选条件下没有考勤规则</div>
      ) : (
            ...
            return (
              <div key={rule.id} className="glass-panel rounded-xl p-5 hover:shadow-md transition-all duration-300">
                ...
                <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
                  <RuleStatCard label="班次时间" value={`${rule.startShift} - ${rule.endShift}`} />
                  <RuleStatCard label="休息时间" value={`${rule.breakStart} - ${rule.breakEnd}`} />
                  <RuleStatCard label="标准工时" value={formatDuration(rule.standardHours)} />
                  <RuleStatCard
                    label="关联员工"
                    value={`${rule.relatedEmployeeCount} 人`}
                ...
                <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs uppercase text-slate-400 mb-1">加班规则</p>
                      <div className="text-sm font-medium text-slate-700 flex items-center gap-2">
                        <Clock3 className="w-4 h-4 text-slate-400" />
                        {rule.overtimeEnabled ? "启用加班计算" : "关闭加班计算"}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xs uppercase text-slate-400 mb-1">加班费标准</p>
                      <p className="text-sm font-semibold text-slate-800">
                        {rule.overtimeEnabled ? formatCurrency(rule.otHourlyFee, "THB") : "不计加班费"}
                      </p>
                    </div>
                  </div>
                  <div className="mt-3 text-xs text-slate-500 rounded-lg bg-slate-50 px-3 py-2">
                    加班计量说明：按 {rule.overtimeMinUnitHours} 小时为最小单位，向下取整。
                  </div>
                </div>
              </div>
            );
