...
const GOOGLE_POPUP_NAME = "wmshr-admin-google-auth";
const GOOGLE_POPUP_POLL_MS = 500;
const GOOGLE_POPUP_QUERY_KEY = "auth_popup";
const GOOGLE_POPUP_QUERY_VALUE = "google";
const GOOGLE_AUTH_MESSAGE_TYPE = "wmshr-admin-google-auth-result";

type GoogleAuthPopupMessage =
  | { type: typeof GOOGLE_AUTH_MESSAGE_TYPE; status: "success" }
  | { type: typeof GOOGLE_AUTH_MESSAGE_TYPE; status: "error"; error: string };
...
function closePopupWindow(target: Window | null | undefined) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data, error }) => {
      ...
      if (isGooglePopupCallback) {
        ...
        const popupError = new URLSearchParams(window.location.search).get("error_description")
  ...
  useEffect(() => {
    ...
    void loadAttendanceRuleModuleData();
  ...
  const handleGoogleLogin = async () => {
    ...
    const redirectTo = `${window.location.origin}/?${GOOGLE_POPUP_QUERY_KEY}=${GOOGLE_POPUP_QUERY_VALUE}`;
  ...
  return (
    <div className="text-slate-700 h-screen flex overflow-hidden font-sans antialiased">
      <Sidebar activeTab={activeTab} onTabChange={setActiveTab} userEmail={session.user.email} />
