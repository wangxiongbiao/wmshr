...
const Nav = ({ onNavigateHome }: { onNavigateHome: () => void }) => {
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "py-4 glass border-b shadow-2xl shadow-black/50" : "py-8"}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-white/70">
          ...
          <LanguageSelector />
          <button className="flex items-center gap-2 px-5 py-2.5 bg-white text-black rounded-full font-bold hover:bg-black/5 transition-all shadow-sm border border-black/5">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            <button className="flex items-center justify-center gap-3 w-full py-4 bg-white text-black rounded-xl font-bold mt-4">
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-brand-primary text-white">
      <Nav onNavigateHome={() => setShowEmailForm(false)} />

      {showEmailForm ? (
        <div className="pt-24 pb-12">
          <EmailFormPage onBack={() => setShowEmailForm(false)} />
        </div>
      ) : (
        <>
          {/* Hero Section */}
