...
export function GoodsManager({ employees, goods, onUpdateGoods, addToast }: GoodsManagerProps) {
  ...
  const [skuList, setSkuList] = useState<{ sku: string; qty: number; desc?: string; actualQty?: number; image?: string }[]>([{ sku: "", qty: 1, desc: "", image: "" }]);
  ...
  const handleOpenAdd = () => {
    ...
    setGoodsName("");
    setSkuList([{ sku: "", qty: 1, desc: "", image: "" }]);
  ...
  const handleOpenEditInbound = (rec: GoodsRecord) => {
    ...
    if (rec.skus && rec.skus.length > 0) {
      setSkuList(rec.skus.map(item => ({ sku: item.sku, qty: item.qty, desc: item.desc || "", image: item.image || "" })));
    } else {
      setSkuList([{ sku: "", qty: 1, desc: "", image: "" }]);
    }
    setIsModalOpen(true);
  ...
  return (
    <div className="space-y-4">
            ...
            <table className="w-full text-left border-collapse min-w-[850px] text-[13px]">
              <thead>
                <tr className="bg-slate-100/90 border-b border-slate-200 text-slate-700 font-bold text-xs uppercase tracking-wider">
                  <th className="px-5 py-3.5">入库单</th>
                            ...
                            <>
                              <div className="space-y-1.5 max-h-[120px] overflow-y-auto pr-1">
                                {rec.skus.map((item, index) => {
                                  ...
                                  return (
                                    <div key={index} className="flex gap-2 text-[11px] bg-slate-50 hover:bg-slate-100/90 px-2 py-1.5 rounded border border-slate-150 transition duration-150 items-start">
                                      {item.image && (
                                        <div 
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setPreviewImageUrl(item.image || null);
                                          }}
                                          className="w-9 h-9 rounded-md overflow-hidden bg-slate-100 border border-slate-200/60 flex-shrink-0 cursor-pointer hover:scale-105 active:scale-95 transition-all shadow-3xs group/sku-thumb relative"
                                          title="点击查看 SKU 大图"
                                        >
                                          <img src={item.image} className="w-full h-full object-cover" referrerPolicy="no-referrer" alt="SKU" />
                                          <div className="absolute inset-0 bg-black/10 opacity-0 group-hover/sku-thumb:opacity-100 transition flex items-center justify-center">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        {item.desc && (
                                          <div className="text-[10px] text-slate-400 font-medium select-all leading-tight border-t border-slate-150 pt-1 mt-1">
                                            中文名称: {item.desc}
                                          </div>
                                        )}
                                      </div>
      ...
      {/* 货物登记/修改弹窗 (重构为独立简洁 of WMS 入库单表单) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-5xl my-4 overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[92vh]">
            ...
            {/* 固定的标题栏 - 变更为品牌主题色 */}
            <div className="bg-brand-600 px-8 py-5.5 text-white flex justify-between items-center flex-shrink-0 border-b border-brand-700">
              ...
              <button
                ...
                className="text-brand-100 hover:text-white hover:bg-white/10 px-4 py-2 rounded-lg text-sm font-bold transition cursor-pointer"
              >
                ✕ 放弃
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="flex-1 flex flex-col overflow-y-auto bg-white">
              <div className="p-8 text-[14px] text-slate-700 leading-relaxed space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div>
                      <label className="block font-bold text-slate-700 mb-2 text-sm tracking-wide">入库单号 <span className="text-rose-500">*</span></label>
                      <input
                        type="text"
                        required
                        ...
                        className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 font-extrabold text-base bg-slate-50/60 focus:bg-white transition disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed tracking-wider"
                      />
                    </div>
                  </div>

                  {/* 麦头与配套发货箱数配置区 */}
                  <div className="space-y-4 bg-amber-50/15 p-5 rounded-xl border border-amber-200 shadow-xs">
                    <div className="flex justify-between items-center pb-2 border-b border-amber-200">
                      <div className="flex items-center gap-2">
                        <span className="font-extrabold text-slate-800 text-sm tracking-wide">麦头与对应发货箱数</span>
                        <span className="text-xs text-slate-500 font-medium">(支持单麦头对应多箱或一一对应)</span>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 text-xs font-bold text-brand-600 hover:text-white bg-white hover:bg-brand-600 px-4 py-2 rounded-xl border border-slate-200 hover:border-brand-600 transition active:scale-95 cursor-pointer shadow-sm"
                        >
                    ...
                    <div className="space-y-3 max-h-[190px] overflow-y-auto pr-1">
                      {shippingMarkList.map((item, index) => (
                        <div key={index} className="flex items-center gap-4 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs hover:border-slate-350 transition animate-in fade-in slide-in-from-top-1 duration-150">
                          
                          {/* 序号 */}
                          <div className="w-7 h-7 rounded-full bg-amber-100 text-amber-800 text-xs font-extrabold flex items-center justify-center flex-shrink-0 border border-amber-200/50">
                            {index + 1}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <input
                              ...
                              className="w-full px-4 py-2.5 text-sm font-semibold text-slate-800 border border-slate-200 rounded-xl outline-none bg-slate-50/20 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 transition"
                            />
                          </div>
                          ...
                          <div className="w-36 flex-shrink-0">
                            <div className="relative rounded-md shadow-xs">
                              <input
                                ...
                                className="w-full pl-4 pr-12 py-2.5 text-sm font-extrabold text-slate-900 border border-slate-200 rounded-xl outline-none bg-slate-50/20 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 transition text-center"
                              />
                              <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <span className="text-slate-550 text-xs font-bold">箱数</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            className="p-2.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition disabled:opacity-35 disabled:hover:bg-transparent cursor-pointer"
                            title="删除此麦头"
                          >
                    ...
                    {/* 汇总提示 */}
                    <div className="flex justify-between items-center bg-amber-50/60 p-3 rounded-xl border border-amber-200 text-xs md:text-sm text-amber-900 font-bold">
                      <span>麦头规格: 已配置 {shippingMarkList.length} 组</span>
                      <span className="text-xs md:text-sm text-amber-900 bg-amber-100 px-4 py-1 rounded-lg border border-amber-200 shadow-3xs">
                  ...
                  <div className="space-y-4 bg-slate-50/50 p-5 rounded-xl border border-slate-200">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-250">
                      <div className="flex items-center gap-2">
                        <span className="font-extrabold text-slate-800 text-sm tracking-wide">SKU 与首期入库数量列表</span>
                        <span className="text-xs text-slate-500 font-medium">(支持输入多个)</span>
                        <span className="text-rose-500 font-bold">*</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setSkuList([...skuList, { sku: "", qty: 1, desc: "", image: "" }])}
                        className="inline-flex items-center gap-1.5 text-xs font-bold text-brand-600 hover:text-white bg-white hover:bg-brand-600 px-4 py-2 rounded-xl border border-slate-200 hover:border-brand-600 transition active:scale-95 cursor-pointer shadow-sm"
                      >
                        <Plus className="w-4 h-4" />
                        <span>新添一行 SKU</span>
                    ...
                    <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1 col-span-full">
                      {skuList.map((item, index) => (
                        <div key={index} className="flex flex-col sm:flex-row gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:border-slate-350 transition animate-in fade-in slide-in-from-top-1 duration-150 items-start">
                          
                          {/* Image Box Upload zone */}
                          <div className="flex-shrink-0 w-full sm:w-auto flex flex-col items-center">
                            <label htmlFor={`sku-file-${index}`} className="block relative w-20 h-20 bg-slate-50 hover:bg-slate-100 border-2 border-dashed border-slate-250 hover:border-brand-500 rounded-xl cursor-pointer overflow-hidden transition group flex flex-col items-center justify-center">
                              {item.image ? (
                                <>
                                  <img src={item.image} className="w-full h-full object-cover" referrerPolicy="no-referrer" alt="SKU" />
                                  <div className="absolute inset-x-0 bottom-0 bg-black/60 transition flex items-center justify-center py-1 text-[10px] text-white font-bold">
                                    更换
                                  </div>
                                </>
                              ) : (
                                <div className="text-center p-1 flex flex-col items-center">
                                  <Upload className="w-4 h-4 text-slate-400 group-hover:text-brand-500 mb-1" />
                                  <span className="text-[10px] font-extrabold text-slate-450 group-hover:text-brand-600 block">SKU图片</span>
                                </div>
                              )}
                              <input
                                id={`sku-file-${index}`}
                            ...
                            <div className="flex gap-2 mt-1">
                              {item.image && (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    list[index].image = "";
                                    setSkuList(list);
                                    addToast(`SKU ${index + 1} 图片已清除`);
                                  }}
                                  className="text-[10px] text-rose-500 hover:text-rose-700 font-extrabold transition cursor-pointer"
                                >
                                  清除
                                </button>
                              )}
                              <button
                                type="button"
                                onClick={() => {
                                  const url = prompt("请输入图片网址 (URL):", item.image || "");
                                  if (url !== null) {
                                    const list = [...skuList];
                                    list[index].image = url.trim();
                                    setSkuList(list);
                                    if (url.trim()) {
                                      addToast(`SKU ${index + 1} 已自动存为网络图片`);
                                    }
                                  }
                                }}
                                className="text-[10px] text-brand-600 hover:text-brand-800 font-bold transition cursor-pointer"
                              >
                                填网址
                              </button>
                            </div>
                          ...
                          {/* SKU fields and descriptions */}
                          <div className="flex-1 w-full space-y-3">
                            
                            {/* 中文SKU名称输入框 */}
                            <div className="flex items-center gap-2 bg-slate-50/60 p-2 rounded-xl border border-slate-100">
                              <span className="text-xs font-bold text-slate-550 whitespace-nowrap">中文SKU名称:</span>
                              <input
                                id={`sku-desc-${index}`}
                                type="text"
                                placeholder="请输入中文SKU名称 (例: 智能穿戴运动手表)"
                                value={item.desc || ""}
                                onChange={(e) => {
                                  const list = [...skuList];
                                ...
                                className="flex-1 px-3 py-2 text-xs md:text-sm font-semibold text-slate-705 border border-slate-200 rounded-lg outline-none bg-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500/10 transition placeholder:font-normal placeholder:text-slate-400"
                              />
                            </div>

                            <div className="flex items-center gap-4">
                              {/* SKU 序号 */}
                              <div className="w-7 h-7 rounded-full bg-slate-100 text-slate-650 text-xs font-extrabold flex items-center justify-center flex-shrink-0 border border-slate-200/80 font-mono">
                                {index + 1}
                              </div>
                              ...
                              <div className="flex-1 min-w-0">
                                <input
                                  ...
                                  className="w-full px-4 py-2.5 text-sm font-semibold text-slate-850 border border-slate-200 rounded-xl outline-none bg-slate-50/20 focus:bg-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 transition"
                                />
                              </div>
                              ...
                              <div className="w-36 flex-shrink-0">
                                <div className="relative rounded-md shadow-xs">
                                  <input
                                    ...
                                    className="w-full pl-4 pr-12 py-2.5 text-sm font-extrabold text-slate-900 border border-slate-200 rounded-xl outline-none bg-slate-50/20 focus:bg-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 transition text-center"
                                  />
                                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                    <span className="text-slate-550 text-xs font-bold">数量</span>
                                  </div>
                                </div>
                              </div>
                  ...
                  <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 mt-2">
                    <div className="mb-2.5">
                      <label className="block font-bold text-slate-700 uppercase tracking-wider text-xs md:text-sm">
                        上传发货入库单货物图片
                        <span className="text-slate-500 font-medium ml-2 text-xs normal-case">(支持拖拽或选择多张图片，点击缩略图可预览)</span>
                      </label>
                    </div>
              ...
              {/* 底部操作页脚 */}
              <div className="bg-slate-50 px-8 py-5 border-t border-slate-200 flex justify-end items-center gap-4 flex-shrink-0">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-6 py-3 rounded-xl border border-slate-300 text-slate-700 bg-white hover:bg-slate-100 transition-all font-bold text-sm cursor-pointer shadow-3xs"
                >
                  取消
                </button>
                <button
                  type="submit"
                  className="px-8 py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-extrabold shadow-md hover:shadow-lg transition-all text-sm cursor-pointer active:scale-[0.98]"
                >
                  保存记录
                </button>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <DatePicker required value={arrivalDate} onChange={(e) => setArrivalDate(e.target.value)} className="w-full text-xs font-semibold bg-white" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-3.5 bg-slate-50/50 p-4 rounded-xl border border-slate-200">
                    ...
                    <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
                      {skuList.map((item, index) => (
                        <div key={index} className="flex flex-col sm:flex-row gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs hover:border-slate-350 transition animate-in fade-in slide-in-from-top-1 duration-150 items-start">
                          
                          {/* Image Box Upload zone/Display zone */}
                          <div className="flex-shrink-0 w-full sm:w-auto flex flex-col items-center">
                            <label htmlFor={`sku-box-file-${index}`} className={`block relative w-16 h-16 bg-slate-50 border-2 border-dashed border-slate-200 rounded-lg overflow-hidden transition group flex flex-col items-center justify-center ${
                              !editingGoods ? "cursor-pointer hover:border-brand-500 hover:bg-slate-100" : ""
                            }`}>
                              {item.image ? (
                                <>
                                  <img 
                                  ...
                                  />
                                  {!editingGoods && (
                                    <div className="absolute inset-x-0 bottom-0 bg-black/60 transition flex items-center justify-center py-0.5 text-[9px] text-white font-bold select-none">
                                      更换
                                    </div>
                                  )}
                                </>
                              ) : (
                                <div className="text-center p-1 flex flex-col items-center select-none">
                                  <Upload className="w-3.5 h-3.5 text-slate-400 group-hover:text-brand-500 mb-0.5" />
                                  <span className="text-[9px] font-bold text-slate-400 group-hover:text-brand-600 block">SKU图片</span>
                                </div>
                              )}
                              {!editingGoods && (
                                <input
                                  id={`sku-box-file-${index}`}
                                  type="file"
                                  accept="image/*"
                                  className="hidden"
                                  disabled={!!editingGoods}
                              ...
                              <div className="flex gap-2 mt-1">
                                {item.image && (
                                  <button
                                    ...
                                    className="text-[9px] text-rose-500 hover:text-rose-700 font-bold transition cursor-pointer"
                                  >
                                    清除
                                  </button>
                                )}
                                <button
                                  ...
                                  className="text-[9px] text-brand-600 hover:text-brand-800 font-bold transition cursor-pointer"
                                >
                                  填网址
                                </button>
                              </div>
                          ...
                          <div className="flex-1 w-full space-y-2">
                            ...
                            {/* 中文SKU名称输入框 */}
                            <div className="flex items-center gap-2 bg-slate-50/40 p-1.5 rounded-lg border border-slate-100">
                              ...
                              <div className="flex-1 min-w-0">
                                <input
                                  ...
                                  className={`w-full px-3 py-2 text-xs font-semibold text-slate-850 border border-slate-200 rounded-lg outline-none bg-slate-50/20 focus:bg-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 transition ${
                                    editingGoods ? "bg-slate-100/80 text-slate-505 cursor-not-allowed select-none border-dashed" : ""
                                  }`}
                                />
                              </div>
