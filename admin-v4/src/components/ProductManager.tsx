...
export function ProductManager({ products, onUpdateProducts, customers, goodsList, addToast }: ProductManagerProps) {
  ...
  const openEditModal = (p: Product) => {
    ...
    setNameInput(p.name);
    setSpecInput(p.spec || "");
    setLengthInput(p.length !== undefined ? String(p.length) : "");
    setWidthInput(p.width !== undefined ? String(p.width) : "");
    setHeightInput(p.height !== undefined ? String(p.height) : "");
    setWeightInput(p.weight !== undefined ? String(p.weight) : "");
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    const vol = (!isNaN(l) && !isNaN(w) && !isNaN(h)) ? (l * w * h) : undefined;
                          ...
                          <tr key={`${prod.id}-${prod.customerId}`} className="hover:bg-slate-50/50 transition">
                            ...
                            <td className="px-5 py-4 max-w-[200px] whitespace-normal">
                              <div className="space-y-1">
                                {prod.length !== undefined && prod.width !== undefined && prod.height !== undefined ? (
                                  <div className="flex flex-col gap-0.5 text-[10.5px]">
                                    <div className="font-mono text-slate-800 font-bold bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded inline-block w-fit">
                                      📏 {prod.length} × {prod.width} × {prod.height} cm
                                    </div>
                                    <div className="font-mono text-indigo-700 font-extrabold text-[10px] pl-0.5">
                                      📦 体积: {(prod.length * prod.width * prod.height).toLocaleString(undefined, { maximumFractionDigits: 1 })} cm³
                                    </div>
                                  </div>
                                ) : (
                                  <span className="text-slate-350 text-[10.5px]">未配置物理尺寸</span>
                                )}
                                {prod.weight !== undefined && (
                                  <div className="font-mono text-emerald-700 font-extrabold text-[10px] pl-0.5">
                                    ⚖️ 重量: {prod.weight} kg
                                  </div>
                                )}
                                {prod.spec && (
                                  <div className="text-[10px] text-slate-400 truncate mt-1 font-semibold pl-0.5" title={prod.spec}>
                                    规格: {prod.spec}
                                  </div>
                                )}
                              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-[60] p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSaveProduct} className="p-6 space-y-5 flex-1 overflow-y-auto text-sm font-semibold">
              ...
              {/* 物理规格与重量指标 (长/宽/高 & 自动算体积) */}
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/70 space-y-3 shadow-3xs">
                <div className="flex items-center gap-1.5 text-slate-800 font-extrabold pb-1.5 border-b border-slate-200/60">
                  <span className="w-1.5 h-3 bg-brand-600 rounded-full"></span>
                  <span className="text-xs uppercase tracking-wider text-slate-700">物理规格与仓容理货指标 (输入尺寸自动计算体积)</span>
                </div>
                
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-450 mb-1">长 (cm)</label>
                    <input
                      type="number"
                      step="any"
                      min="0"
                      ...
                      placeholder="长 (cm)"
                      className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg outline-none text-xs focus:ring-2 focus:ring-brand-500/10 focus:border-brand-500 font-extrabold text-slate-800 font-mono"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-450 mb-1">宽 (cm)</label>
                    <input
                      type="number"
                      step="any"
                      min="0"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-450 mb-1">高 (cm)</label>
                    <input
                      type="number"
                      step="any"
                      min="0"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-450 mb-1">单件重量 (kg)</label>
                    <input
                      type="number"
                      step="any"
                      min="0"
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-indigo-50/50 border border-indigo-100 rounded-lg p-2.5 flex items-center justify-between text-xs select-none">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-indigo-750">自动测算单件体积:</span>
                        <span className="font-black font-mono text-indigo-900 bg-indigo-100/80 px-2 py-0.5 rounded text-xs">
                          {volCm3.toLocaleString(undefined, { maximumFractionDigits: 2 })} cm³
                        </span>
                        <span className="font-medium text-slate-500 font-mono text-[10.5px]">
                          (≈ {volM3.toLocaleString(undefined, { minimumFractionDigits: 4, maximumFractionDigits: 6 })} m³)
                        </span>
                      </div>
                      <div className="text-[10px] text-indigo-600 font-medium font-mono">
                        {l > 0 && w > 0 && h > 0 ? "✓ 体积测算完成" : "等待输入尺寸"}
                      </div>
                    </div>
                  );
                })()}
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* Spec */}
                <div>
                  <label className="block font-bold text-slate-500 mb-1.5 uppercase tracking-wider text-xs">其他辅属特征说明 (颜色/细分/型号等)</label>
                  <input
                    type="text"
                    value={specInput}
                    onChange={(e) => setSpecInput(e.target.value)}
                    placeholder="如: 9色/盒, 双层精装版等"
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl outline-none text-sm focus:ring-2 focus:ring-brand-500/10 focus:border-brand-500 font-bold text-slate-850"
                  />
                </div>
