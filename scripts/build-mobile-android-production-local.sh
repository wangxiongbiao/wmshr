...
# 可选：
#   - REACT_NATIVE_ARCHITECTURES=arm64-v8a 可只构建真机验证用 ABI；默认不设置时仍构建完整多 ABI 侧载包。
# 影响：
...
# Release 产物必须强制重跑 JS bundle 任务，避免 Gradle/Metro 增量缓存复用旧的
# assets/index.android.bundle，导致侧载包的原生版本和 App 内展示版本分叉。
