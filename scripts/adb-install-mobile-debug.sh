...
# WMSHR 本地 Android Debug APK 安装脚本。
# 作用：把 apps/mobile 本地构建出来的 debug APK 安装到当前已连接的安卓设备；可优先配合无线 ADB 目标使用。
# 前提：
#   1. `apps/mobile/android/app/build/outputs/apk/debug/app-debug.apk` 已存在；
...
#   - APK 是否已经通过 `npm --workspace @wmshr/mobile run build:android:local` 产出；
#   - 若报 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`，先卸载手机上旧的 `com.wmshr.app`。
# 边界：
#   - 这里固定安装 debug APK，不处理 release 签名包或 Play 提交；
...
usage() {
  cat <<'EOF'
Usage: bash scripts/adb-install-mobile-debug.sh [options]
...
if [[ ! -f "$APK_PATH" ]]; then
  ...
  echo "Build it first with: npm --workspace @wmshr/mobile run build:android:local" >&2
  exit 1
fi
